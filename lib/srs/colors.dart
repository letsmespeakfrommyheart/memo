import 'package:flutter/material.dart';

Color greenColor = const Color.fromARGB(255, 172, 175, 131);
Color redColor = const Color.fromARGB(255, 227, 19, 42);
Color milkColor = const Color.fromARGB(255, 249, 240, 212);
Color tealColor = const Color.fromARGB(255, 63, 94, 106);
Color greyColor = const Color.fromARGB(255, 59, 55, 58);
Color lightGreyColor = const Color.fromARGB(148, 59, 55, 58);
Color whiteColor = const Color.fromARGB(255, 255, 255, 255);
Color goldColor = const Color.fromARGB(255, 255, 193, 7);

Color deepTeal = const Color.fromARGB(255, 38, 70, 83);
Color lightTeal = const Color.fromARGB(255, 42, 157, 143);
Color green = const Color.fromARGB(255, 138, 177, 125);
Color yellow = const Color.fromARGB(255, 233, 196, 106);
Color lightOrange = const Color.fromARGB(255, 244, 162, 97);
Color deepOrange = const Color.fromARGB(255, 231, 111, 81);
Color deepPink = const Color.fromARGB(255, 238, 70, 98);
Color blackColor = const Color.fromARGB(255, 0, 0, 0);

final studyCardBackgroundColors = [
  greenColor,
  lightOrange,
  yellow,
];
